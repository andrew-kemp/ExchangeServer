...
###################################################
# Logging and Settings Functions                 #
###################################################

# Get the script directory for log and settings files
$script:ScriptPath = Split-Path -Parent $MyInvocation.MyCommand.Path
$script:LogFile = Join-Path $ScriptPath "ExchangeCert_Audit.log"
$script:SettingsFile = Join-Path $ScriptPath "ExchangeCert_Settings.ini"

function Write-AuditLog {
    <#
    .SYNOPSIS
    Writes timestamped audit information to the log file
    .PARAMETER Message
    The message to log
    .PARAMETER Type
    The type of log entry (INFO, ERROR, WARNING)
    #>
    param(
        [string]$Message,
        [string]$Type = "INFO"
    ...
    try {
        ...
        # Silently handle logging errors to avoid disrupting the main application
    }
}
...
function Load-Settings {
    <#
    .SYNOPSIS
    Loads settings from the INI file and populates form fields
    .DESCRIPTION
    Reads key=value pairs from ExchangeCert_Settings.ini and sets corresponding form values
    #>
    
    try {
        if (-not (Test-Path $script:SettingsFile)) {
            Write-AuditLog "Settings file not found, using default values" "INFO"
            return
        }
        
        Write-AuditLog "Loading settings from $script:SettingsFile" "INFO"
        
        $settingsContent = Get-Content $script:SettingsFile -ErrorAction Stop
        foreach ($line in $settingsContent) {
            if ($line -and $line.Contains('=') -and -not $line.StartsWith('#')) {
                ...
                switch ($key) {
                    'Username' { if ($txtUsername) { $txtUsername.Text = $value } }
                    'ExchangeServer' { if ($txtServer) { $txtServer.Text = $value } }
                    'CertificateType' { 
                        if ($value -eq 'Wildcard' -and $rbWildcard) { 
                            $rbWildcard.Checked = $true 
                        } elseif ($rbSAN) { 
                            $rbSAN.Checked = $true 
                        }
                    }
                    'FriendlyName' { if ($txtFriendly) { $txtFriendly.Text = $value } }
                    'SubjectName' { if ($txtCN) { $txtCN.Text = $value } }
                    'SANs' { if ($txtSAN) { $txtSAN.Text = $value } }
                }
            }
        }
        Write-AuditLog "Settings loaded successfully" "INFO"
    } catch {
        Write-AuditLog "Error loading settings: $($_.Exception.Message)" "ERROR"
    }
}
...
function Save-Settings {
    <#
    .SYNOPSIS
    Saves current form settings to the INI file
    .DESCRIPTION
    Writes current form field values as key=value pairs to ExchangeCert_Settings.ini
    #>
    
    try {
        Write-AuditLog "Saving current settings to $script:SettingsFile" "INFO"
        
        $settings = @()
        $settings += "# ExchangeCert Management Settings"
        $settings += "# This file is automatically generated and updated"
        $settings += ""
        
        if ($txtUsername) { $settings += "Username=$($txtUsername.Text)" }
        if ($txtServer) { $settings += "ExchangeServer=$($txtServer.Text)" }
        
        if ($rbWildcard -and $rbWildcard.Checked) {
            $settings += "CertificateType=Wildcard"
        } else {
            $settings += "CertificateType=SAN"
        }
        
        if ($txtFriendly) { $settings += "FriendlyName=$($txtFriendly.Text)" }
        if ($txtCN) { $settings += "SubjectName=$($txtCN.Text)" }
        if ($txtSAN) { $settings += "SANs=$($txtSAN.Text)" }
        
        $settings | Out-File -FilePath $script:SettingsFile -Encoding UTF8 -Force
        Write-AuditLog "Settings saved successfully" "INFO"
    } catch {
        Write-AuditLog "Error saving settings: $($_.Exception.Message)" "ERROR"
    }
}
...
function EnsureExchangeConnection {
    ...
    Write-AuditLog "Attempting to connect to Exchange server: $server with username: $username" "INFO"
    
    if (-not $username -or -not $password -or -not $server) {
        Write-AuditLog "Connection failed: Missing required connection fields" "ERROR"
    ...
    try {
        ...
        Write-AuditLog "Successfully connected to Exchange server: $server" "INFO"
        ...
        Write-AuditLog "Connection to Exchange failed: $($_.Exception.Message)" "ERROR"
...
$btnDisconnect.Add_Click({
    Write-AuditLog "User initiated disconnect from Exchange" "INFO"
    ...
    Write-AuditLog "Successfully disconnected from Exchange" "INFO"
})
...
# Event handlers for certificate type radio buttons to update UI and save settings
$rbSAN.Add_CheckedChanged({
    if ($rbSAN.Checked) {
        $lblCN.Text = "Subject Name (e.g. CN=mail.domain.com):"
        $lblSAN.Enabled = $true
        $txtSAN.Enabled = $true
        # Save settings when certificate type changes
        Save-Settings
    }
})
...
$btnGenCSR.Add_Click({
    ...
    Write-AuditLog "User initiated CSR generation" "INFO"
    ...
    if (-not $global:Connected -or -not $global:Session) {
        ...
        Write-AuditLog "CSR generation failed: Not connected to Exchange" "ERROR"
        return
    }
    ...
    Write-AuditLog "CSR parameters - Friendly: $friendly, Subject: $subject, SAN: $san, Type: $(if($isWildcard){'Wildcard'}else{'SAN'})" "INFO"
    
    if (-not $friendly -or -not $subject -or -not $reqPath) {
        $lblCSRResult.ForeColor = [System.Drawing.Color]::Red
        $lblCSRResult.Text = "All fields except SAN are required."
        Write-AuditLog "CSR generation failed: Missing required fields" "ERROR"
        return
    }
    if ($isWildcard -and ($subject -notmatch "^CN=\*\.") ) {
        $lblCSRResult.ForeColor = [System.Drawing.Color]::Red
        $lblCSRResult.Text = "Wildcard subject must begin with CN=*."
        Write-AuditLog "CSR generation failed: Invalid wildcard subject format" "ERROR"
        return
    }
    try {
        $params = @{
        ...
        Write-AuditLog "CSR generated successfully and saved to: $reqPath" "INFO"
        # Save settings after successful CSR generation
        Save-Settings
        
    } catch {
        $lblCSRResult.ForeColor = [System.Drawing.Color]::Red
        $lblCSRResult.Text = "Error: $($_.Exception.Message)"
        Write-AuditLog "CSR generation failed: $($_.Exception.Message)" "ERROR"
    }
})
...
$btnComplete.Add_Click({
    ...
    Write-AuditLog "User initiated certificate completion" "INFO"
    ...
    if (-not $global:Connected -or -not $global:Session) {
        ...
        Write-AuditLog "Certificate completion failed: Not connected to Exchange" "ERROR"
        return
    }
    ...
    if (-not $p7bPath -or -not (Test-Path $p7bPath)) {
        ...
        Write-AuditLog "Certificate completion failed: Invalid or missing .p7b file path: $p7bPath" "ERROR"
        return
    }
    
    Write-AuditLog "Completing certificate request with file: $p7bPath" "INFO"
    ...
    try {
        ...
        Write-AuditLog "Certificate completed successfully with thumbprint: $thumbprint" "INFO"
        
    } catch {
        $lblCompleteResult.ForeColor = [System.Drawing.Color]::Red
        $lblCompleteResult.Text = "Error: $($_.Exception.Message)"
        Write-AuditLog "Certificate completion failed: $($_.Exception.Message)" "ERROR"
    }
})
...
$btnAssign.Add_Click({
    ...
    Write-AuditLog "User initiated certificate assignment" "INFO"
    ...
    if (-not $global:Connected -or -not $global:Session) {
        ...
        Write-AuditLog "Certificate assignment failed: Not connected to Exchange" "ERROR"
        return
    }
    if (-not $global:ImportedThumbprint) {
        ...
        Write-AuditLog "Certificate assignment failed: No imported certificate thumbprint found" "ERROR"
        return
    }
    $services = @()
    ...
    if ($services.Count -eq 0) {
        ...
        Write-AuditLog "Certificate assignment failed: No services selected" "ERROR"
        return
    }
    $svcText = $services -join ","
    $force = $chkForceAssign.Checked
    
    Write-AuditLog "Assigning certificate (thumbprint: $global:ImportedThumbprint) to services: $svcText, Force: $force" "INFO"
    ...
    try {
        ...
        Write-AuditLog "Certificate assigned successfully to services: $svcText" "INFO"
        
    } catch {
        $lblAssignResult.ForeColor = [System.Drawing.Color]::Red
        $lblAssignResult.Text = "Error: $($_.Exception.Message)"
        Write-AuditLog "Certificate assignment failed: $($_.Exception.Message)" "ERROR"
    }
})
...
function Show-ExportCertsForm {
    ...
    $btnExport.Add_Click({
        ...
        if ($lvCerts.SelectedItems.Count -ne 1) {
            ...
            Write-AuditLog "Certificate export failed: No certificate selected" "ERROR"
            return
        }
        ...
        if ($saveDialog.ShowDialog() -ne "OK") { 
            Write-AuditLog "Certificate export cancelled by user" "INFO"
            return 
        }
        $filePath = $saveDialog.FileName

        Write-AuditLog "Initiating certificate export for thumbprint: $thumbprint to file: $filePath" "INFO"

        $pwd = Prompt-ForPassword
        if (-not $pwd) {
            $lblStatus.ForeColor = [System.Drawing.Color]::Red
            $lblStatus.Text = "Export cancelled (password not entered)."
            Write-AuditLog "Certificate export cancelled: Password not provided" "INFO"
            return
        }
        try {
            ...
            Write-AuditLog "Certificate exported successfully to: $filePath" "INFO"
        } catch {
            $lblStatus.ForeColor = [System.Drawing.Color]::Red
            $lblStatus.Text = "Export failed: $($_.Exception.Message)"
            Write-AuditLog "Certificate export failed: $($_.Exception.Message)" "ERROR"
        }
    })
...
$btnExportCerts.Add_Click({
    Write-AuditLog "User initiated export certificates dialog" "INFO"
    
    if (-not $txtUsername.Text -or -not $txtPassword.Text -or -not $txtServer.Text) {
        Write-AuditLog "Export certificates failed: Missing connection details" "ERROR"
    ...
    if (-not $global:Connected -or -not $global:Session) {
        Write-AuditLog "Export certificates failed: Not connected to Exchange" "ERROR"
        [System.Windows.Forms.MessageBox]::Show(
...
function Show-ImportCertForm {
    ...
    $btnImport.Add_Click({
        ...
        Write-AuditLog "Initiating certificate import from file: $file to servers: $($selectedServers -join ',') for services: $($services -join ',')" "INFO"
        
        if (-not (Test-Path $file)) {
            $lblStatus.ForeColor = [System.Drawing.Color]::Red
            $lblStatus.Text = "PFX file not found."
            Write-AuditLog "Certificate import failed: PFX file not found at path: $file" "ERROR"
            return
        }
        if ($selectedServers.Count -eq 0) {
            $lblStatus.ForeColor = [System.Drawing.Color]::Red
            $lblStatus.Text = "Please select at least one Exchange server."
            Write-AuditLog "Certificate import failed: No Exchange servers selected" "ERROR"
            return
        }
        if ($services.Count -eq 0) {
            $lblStatus.ForeColor = [System.Drawing.Color]::Red
            $lblStatus.Text = "Please select at least one service."
            Write-AuditLog "Certificate import failed: No services selected" "ERROR"
            return
        }
        $securePwd = ConvertTo-SecureString $pwd -AsPlainText -Force
        $fileData = $null
        try {
            ...
            Write-AuditLog "Certificate import failed: Could not read PFX file: $($_.Exception.Message)" "ERROR"
            return
        }
        $success = $true
        $importedThumbprints = @()
        ...
        foreach ($server in $selectedServers) {
            try {
                ...
                Write-AuditLog "Certificate imported and assigned successfully on server: $server with thumbprint: $thumb" "INFO"
            } catch {
                $success = $false
                $lblStatus.ForeColor = [System.Drawing.Color]::Red
                $lblStatus.Text = "Failed for ${server}: $($_.Exception.Message)"
                Write-AuditLog "Certificate import failed for server ${server}: $($_.Exception.Message)" "ERROR"
                break
            }
        }
        if ($success -and $importedThumbprints.Count -gt 0) {
            ...
            Write-AuditLog "Certificate import operation completed successfully for all selected servers" "INFO"
            $importForm.Close()
        }
    })
...
$btnImportCert.Add_Click({
    Write-AuditLog "User initiated import certificate dialog" "INFO"
    
    if (-not $global:Connected -or -not $global:Session) {
        Write-AuditLog "Import certificate failed: Not connected to Exchange" "ERROR"
        [System.Windows.Forms.MessageBox]::Show(
            "Please connect to Exchange first.",
            "Connection Required",
            [System.Windows.Forms.MessageBoxButtons]::OK,
...
$form.Add_Shown({
    # Log script startup
    Write-AuditLog "ExchangeCert Management script started" "INFO"
